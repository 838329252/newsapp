...
//        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (isInPlaybackState() && mMediaController != null) {
            ...
            Log.d("seeTouchEvent","is touch event play");
        }
        return false;
    }
    ...
    /*private void setVolume(float value) {
        TextView volumeTextView=(TextView)findViewById(R.id.volume);
        // 获取音量管理器
        AudioManager manager = (AudioManager) getContext().getSystemService(AUDIO_SERVICE);
        // 获取当前音量
        int curretnVolume= manager.getStreamVolume(AudioManager.STREAM_MUSIC);
        int maxVolume = manager.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
        int flag = value > 0 ? -1 : 1;
        curretnVolume += flag * 0.15 * maxVolume;
        manager.setStreamVolume(AudioManager.STREAM_MUSIC, curretnVolume, AudioManager.FLAG_SHOW_UI);
        volumeTextView.setVisibility(View.VISIBLE);
        volumeTextView.setText("音量:" + (float) Math.ceil((curretnVolume/ 16.0) * 100) + "%");
    }*/
}
